...
public class CursoController {
    ...
    public ResponseEntity<?> crear(@RequestBody Curso e) {
        if (e.getNombre().isEmpty() || e.getDescripcion().isEmpty() || e.getNivel().equals(0) || e.getProf_id().equals(0)) {
            return new ResponseEntity<>("Deben estar llenos todos los campos", HttpStatus.BAD_REQUEST);
        //.matches("[a-zA-Z]")
        }
        if (!e.getNombre().matches("[a-zA-Z]*") || !e.getDescripcion().matches("[a-zA-Z]*")) {
    ...
    public ResponseEntity<Curso> actualizar(@PathVariable Long id, @RequestBody Curso e) {
